package com.example.arivald.eventbus;
 ...
 *
 * TODO add "sticky" events - sticky event is delivered to all existing receivers, and to all new receivers in specific amount of time.
 */
...
public final class EventBus {
    ...
    public enum DeliveryThread {
         ...
         * The thread have to have an associated {@link Looper} (Looper.myLooper() should not be null)
         * at the time the register() is called.
         * If there is no {@link Looper}, this is equivalent of the DeliveryThread.BACKGROUND.
         ...
         * Best to ue with the {@link HandlerThread}.
     ...
     */
    public EventBus(@NonNull String name, @Nullable Executor executor) {
        mName = name;
        HandlerThread thread = new HandlerThread(toString() + ".dispatcher", android.os.Process.THREAD_PRIORITY_BACKGROUND);
        thread.start();
        mDispatcherThreadHandler = new Handler(thread.getLooper());
    ...
    public void assignThreadForTarget(@NonNull Object target) {
        ...
        if (looper != null)
            synchronized (mTargetsLoopers) {
                ...
                mTargetsLoopers.put(target, new WeakReference<>(looper));
    ...
    public void unregister(@Nullable Object target) {
        ...
        //remove looper and handler
        synchronized (mTargetsLoopers) {
            WeakReference<Looper> looperRef = mTargetsLoopers.get(target);
            mTargetsLoopers.remove(target);
            if (looperRef != null) {
                final Looper targetLooper = looperRef.get();
                if (!isLooperReferenced(targetLooper)) {
                    mLoopersHandlers.remove(targetLooper);
                }
            }
        }
    }

    private boolean isLooperReferenced(@NonNull Looper looper) {
        if (looper == Looper.getMainLooper()) {
            return true;
        }
        synchronized (mTargetsLoopers) {
            for (Map.Entry<Object, WeakReference<Looper>> looperRef : mTargetsLoopers.entrySet()) {
                if (looper == looperRef.getValue().get()) {
                    return true;
                }
            }
            return false;
     ...
     */
    final Map<Class<?>, CopyOnWriteArraySet<Subscriber>> mCurrentlyRegisteredSubscribersByEventType = new HashMap<>();

    /**
     * target-to-looper map.
     */
    final static Map<Object, WeakReference<Looper>> mTargetsLoopers = new WeakHashMap<>();

    /**
     * looper-to-handler map.
     */
    final static Map<Looper, Handler> mLoopersHandlers = new WeakHashMap<>();
     ...
     */
    static final Map<Class<?>, Map<Class<?>, MethodDescLinkedListItem>> mSubscriberMethodsCache = new HashMap<>();
    ...
    static Map<Class<?>, MethodDescLinkedListItem> scanForSubscriberMethods(@NonNull final Class<?> listenerClass) {
        Map<Class<?>, MethodDescLinkedListItem> result = new HashMap<>();
    ...
    static Map<Class<?>, Set<Subscriber>> findAllSubscribers(@NonNull final Object target) {
        ...
        List<Map<Class<?>, MethodDescLinkedListItem>> allMethods = new LinkedList<>();
        synchronized (mSubscriberMethodsCache) {
            Class<?> currentClass = target.getClass();
            while (currentClass != null) {
                Map<Class<?>, MethodDescLinkedListItem> methods = mSubscriberMethodsCache.get(currentClass);
                if (methods == null) {
                    methods = scanForSubscriberMethods(currentClass);
                    mSubscriberMethodsCache.put(currentClass, methods);
                }
        ...
        for (Map<Class<?>, MethodDescLinkedListItem> methods : allMethods) {
            //Key: event class
            ...
            for (Map.Entry<Class<?>, MethodDescLinkedListItem> e : methods.entrySet()) {
                //e.getKey() => class of the event
                Set<Subscriber> subscribers = result.get(e.getKey());
                if (subscribers == null) {
